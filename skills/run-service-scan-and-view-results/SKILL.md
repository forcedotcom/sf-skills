> **Mirrored skill.** This is a derived, prose-only copy of [`run-service-scan-and-view-results`](https://github.com/mulesoft/mulesoft-dx/tree/master/skills/run-service-scan-and-view-results) in `mulesoft-dx`, generated by `mulesoft-dx-sf-skills-export`. mulesoft-dx is the source of truth -- its version is machine-validated against live Anypoint API specs; this one is not.
...
- POST call (`executeWorkflowFromConfiguration`): Execute workflow from scanner configuration
...
- GET call (`getScannerRunHistory`): Get scanner run history
...
- GET call (`getStagingAssetsByScanRunId`): Get staging assets for a scan run
