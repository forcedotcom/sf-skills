...
- `assetId`: user-provided (example: `my-mcp-server`) -- A unique identifier for the asset in kebab-case
...
**Inputs:**
- `types`: fixed value `mcp` -- Filter for MCP server assets only

**Outputs:**
- `groupId` (`$[*].groupId`, label `$[*].name`): Group ID of the selected MCP server asset
- `assetId` (`$[*].assetId`): Asset ID of the selected MCP server asset
- `assetVersion` (`$[*].version`): Version of the selected MCP server asset
...
- `instanceLabel`: user-provided (example: `my-mcp-server-v1`) -- A human-readable label for this API instance (e.g., "my-mcp-server-v1")
...
- `endpoint.uri`: user-provided, optional (example: `https://backend.example.com/mcp/v1`) -- The upstream backend URL for the MCP server. Ask the user if they want to provide it now or configure it later.
...
- `organizationId`: from `listMe` (`urn:api:access-management`), field `$.user.organization.id` -- Organization ID
- `environmentId`: from step output `environmentId` -- Environment ID from Step 3
- `family`: fixed value `agentic` -- Filter for MCP server (agentic) instances only

**Outputs:**
- `environmentApiId` (`$.assets[*].apis[*].id`, label `$.assets[*].apis[*].autodiscoveryInstanceName`): The MCP server instance ID in API Manager
