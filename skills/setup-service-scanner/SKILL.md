> **Mirrored skill.** This is a derived, prose-only copy of [`setup-service-scanner`](https://github.com/mulesoft/mulesoft-dx/tree/master/skills/setup-service-scanner) in `mulesoft-dx`, generated by `mulesoft-dx-sf-skills-export`. mulesoft-dx is the source of truth -- its version is machine-validated against live Anypoint API specs; this one is not.
...
- GET call (`getTargetSystems`): Get target systems
...
- POST call (`testConnection`): Test connection
...
- POST call (`createScanConfigurations`): Create Scanner Configurations
