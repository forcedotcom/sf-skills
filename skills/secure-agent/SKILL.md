> **Mirrored skill.** This is a derived, prose-only copy of [`secure-agent`](https://github.com/mulesoft/mulesoft-dx/tree/master/skills/secure-agent) in `mulesoft-dx`, generated by `mulesoft-dx-sf-skills-export`. mulesoft-dx is the source of truth -- its version is machine-validated against live Anypoint API specs; this one is not.
...
- **Full setup**: Steps 1, 2, 3, 4, 5, 6, 8, 9
  - When: You have an agent specification and need to publish it to Exchange first
  - You'll need: `organizationId`, agent specification file

- **From Exchange asset**: Steps 2, 3, 4, 5, 6, 8, 9
  - When: The agent is already published in Exchange and you need to find it and create an API Manager instance
  - You'll need: `organizationId`

- **Apply policy only**: Steps 3, 7, 8, 9
  - When: You already have an API Manager instance for the agent and want to apply a policy
  - You'll need: `organizationId`


## Step 1: Publish Agent to Exchange

Publishes your agent specification to Exchange as a reusable asset. This makes it available for API Manager to create managed instances from it.
...
**Action:** Create a new asset in Exchange with your agent specification.

- POST call (`uploadAsset`): Upload asset

**What happens next:** Your agent specification is now available in Exchange. Next, you can search for it to confirm and extract the coordinates needed for API Manager.


## Step 2: Find Agent in Exchange

Search Exchange for agent assets. This search is not scoped to a specific organization — it returns all agent assets visible to the authenticated user across all organizations.

**What you'll need:**
- Valid Bearer token (from Prerequisites)

**Action:** Search Exchange filtering by agent asset type and select the target agent.

- GET call (`searchAssets`): Search assets

**What happens next:** You have the Exchange coordinates (groupId, assetId, version) for the agent. Next, select an environment where the API instance will be created.


## Step 3: List Environments

List all environments in the organization so you can confirm or select the one where your agent instance will live.
...
- GET call (`listEnvironments`): List environments
...
- GET call (`getGatewayTargets`): List gateway targets
...
- POST call (`createApiInstance`): Create an API
...
- POST call (`createProxyDeployment`): Create proxy deployment
...
Lists agent instances in the selected environment by filtering with `family=agentic`. Use this step when you already have an API Manager instance for the agent and need to find its ID to apply a policy.

**What you'll need:**
- Organization ID and Environment ID

**Action:** List agent instances and select the target.

- GET call (`listApiInstances`): List APIs

**What happens next:** You have the agent instance ID. Next, browse the policy catalog to select which policy to apply.
...
- GET call (`getExchangePolicyTemplates`): List exchange policy templates
...
Apply the selected policy to your agent instance with the appropriate configuration. Use the Exchange coordinates and configuration property names from Step 8.
...
- POST call (`applyApiInstancePolicy`): Apply a new API policy
...
### Agent Discovery
- The Exchange search in Step 2 is not org-scoped — it returns all agents visible to the authenticated user
- Use the asset `name` field to identify the correct agent when multiple results are returned
- If you published the agent in Step 1, its coordinates are already available — Step 2 can be used to confirm
...
### Agent Not Found in Exchange

**Symptoms:** Step 2 returns an empty list

**Possible causes:**
- The agent has not been published to Exchange yet
- The asset type is not `agent`

**Solutions:**
- Start from Step 1 to publish the agent to Exchange first
