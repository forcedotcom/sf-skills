> **Mirrored skill.** This is a derived, prose-only copy of [`apply-policy-to-api-instance`](https://github.com/mulesoft/mulesoft-dx/tree/master/skills/apply-policy-to-api-instance) in `mulesoft-dx`, generated by `mulesoft-dx-sf-skills-export`. mulesoft-dx is the source of truth -- its version is machine-validated against live Anypoint API specs; this one is not.
...
- GET call (`listMe`): Get caller information
