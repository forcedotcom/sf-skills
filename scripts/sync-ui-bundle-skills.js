#!/usr/bin/env node
/**
 * Sync UI bundle skills: pins b2e and b2x template packages to latest npm versions,
...
function addUiBundlePrefix(name) {
  if (name.includes('salesforce-ui-bundle')) return name;
  const parts = name.split('-');
  if (parts.length < 2) return name;
  return parts[0] + '-salesforce-ui-bundle-' + parts.slice(1).join('-');
}

/** Dirs in skills/ that look like synced UI bundle skills (e.g. *-ui-bundle-*, creating-ui-bundle). */
function isSyncedUiBundleSkillDir(name) {
  return name.includes('ui-bundle');
}
...
const currentDestNames = new Set(srcNames.map(addUiBundlePrefix));

for (const name of fs.readdirSync(skillsDir)) {
  const dirPath = path.join(skillsDir, name);
  if (!fs.statSync(dirPath).isDirectory()) continue;
  if (!isSyncedUiBundleSkillDir(name)) continue;
...
for (const srcName of srcNames) {
  ...
  const destName = addUiBundlePrefix(srcName);
