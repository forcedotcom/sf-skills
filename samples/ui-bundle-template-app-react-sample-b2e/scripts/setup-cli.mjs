 ...
 *   2. ui-bundle — (all UI bundles) npm install && npm run build so dist exists for deploy (skip with --skip-ui-bundle-build)
...
const UI_BUNDLES_DIR = resolve(SFDX_SOURCE, 'uiBundles');
...
Options:
  --ui-bundle-name <name>   Web app folder name under uiBundles/ (default: auto-detect)
...
function discoverAllUiBundleDirs(uiBundleName) {
  if (!existsSync(UI_BUNDLES_DIR)) {
    console.error(`Error: uiBundles directory not found: ${UI_BUNDLES_DIR}`);
    process.exit(1);
  }
  const entries = readdirSync(UI_BUNDLES_DIR, { withFileTypes: true });
  const dirs = entries.filter((e) => e.isDirectory() && !e.name.startsWith('.'));
  if (dirs.length === 0) {
    console.error(`Error: No UI bundle folder found under ${UI_BUNDLES_DIR}`);
    process.exit(1);
  }
  if (uiBundleName) {
    ...
    return [resolve(UI_BUNDLES_DIR, requested.name)];
  }
  return dirs.map((d) => resolve(UI_BUNDLES_DIR, d.name));
}
...
async function main() {
  ...
  console.log(
    'Steps: login=%s deploy=%s permset=%s data=%s graphql=%s ui-bundle=%s dev=%s',
    !skipLogin,
    !skipDeploy,
    !skipPermset,
    doData,
